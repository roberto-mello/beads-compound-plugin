---
name: beads:research
description: Deep research using specialized agents with findings logged to beads and knowledge
argument-hint: "[bead ID] [research question]"
---

# Beads Research

Conduct deep research on a topic using multiple specialized agents, with all findings logged to a bead and persistent knowledge.
...
Dispatch all 5 research agents in parallel:
...
Task(subagent_type="learnings-researcher",
     prompt="BEAD_ID: {BEAD_ID}

Search the knowledge base for relevant learnings about: {question}

Search .beads/memory/knowledge.jsonl for:
- Previous solutions to similar problems
- Documented gotchas and constraints
- Patterns that worked or failed
- Related decisions and their rationale

Log relevant findings as:
bd comment add {BEAD_ID} \"INVESTIGATION: {relevant past knowledge}\"")
```

### Step 4: Optional SpecFlow Analysis

If the research question relates to a feature or workflow, offer SpecFlow analysis:

Use **AskUserQuestion tool**: "Would you like a SpecFlow analysis to validate the feature specification?"

If yes:
- Task spec-flow-analyzer(question, research_findings)
...
2. Organize by type:
   ...
   - **Investigations** (how things currently work, past knowledge)
...
Past Knowledge Applied:
{relevant learnings from knowledge.jsonl}
...
- All research findings are auto-captured as persistent knowledge
- 5 agents run in parallel: best-practices, framework-docs, git-history, repo-research, learnings
